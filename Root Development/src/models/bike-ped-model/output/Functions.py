...
def h_o(health_outcome):
    df = pd.read_csv('bike_ped_model_sweep.csv')
...
def min_mileage_for_max_outcome(census_tract, health_outcome):
    df = pd.read_csv('bike_ped_model_sweep.csv')
    #applying function to return the median value for the given health outcome
    df = h_o(health_outcome)
...
def min_mileage_any_outcome_improvement(census_tract, health_outcome):
    df = pd.read_csv('bike_ped_model_sweep.csv')
    #applying function to return the median value for the given health outcome
    df = h_o(health_outcome)
    #filtering only the values corresponding to the input census tract value
...
def bottom_top_diff_improvement(top_prct, bottom_prct, health_outcome):
    df = pd.read_csv('bike_ped_model_sweep.csv')
